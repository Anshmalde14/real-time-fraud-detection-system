...
while True:
    ...
    for txn in transactions:
        if txn is None:
            continue
        txn["transaction_gap_seconds"] = random.randint(1,1000)
        txn["transactions_last_5min"] = random.randint(0,10)
        txn["device_change"] = random.randint(0,1)
        txn["amount_balance_ratio"] = round(txn["amount"] /max(txn["remaining_balance"], 1),4)
        txn["transaction_hour"] = txn["timestamp"].hour
        txn["is_high_amount"] = int(txn["amount"] > 5000)
        txn["user_type_encoded"] = random.randint(0,3)
        txn["merchant_category_encoded"] = random.randint(0,4)
        txn["merchant_encoded"] = random.randint(0,14)
        txn["location_encoded"] = random.randint(0,10)
        txn["device_type_encoded"] = random.randint(0,2)
        ...
        try:
            ...
            txn["fraud_probability"] = (prediction["fraud_probability"])
            txn["risk_level"] = (prediction["risk_level"])
            txn["prediction"] = (prediction["fraud_prediction"])
            import pandas as pd
            live_df = pd.DataFrame([txn])
            live_df.to_csv("live_transactions.csv",
                mode="a",
