...
def inject_account_drain(transaction, user):
    large_amount = round(user.account_balance * random.uniform(0.7, 0.95),2)
    transaction["amount"] = large_amount
    transaction["old_balance"] = user.account_balance
    transaction["new_balance"] = round(user.account_balance - large_amount,2)
...
def inject_card_testing_fraud(transaction, user):
    transaction["amount"] = round(random.uniform(1, 20),2)
